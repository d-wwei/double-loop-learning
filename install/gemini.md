...
with open("cognitive-protocol.md") as f:
    double_loop_rules = f.read()

model = genai.GenerativeModel(
    model_name="gemini-2.0-flash",
    system_instruction=double_loop_rules
)
```
...
Place First Principles first in the system instruction, then Double-Loop Learning. Both are concise enough to fit together without significant context cost.
